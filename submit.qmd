...
# Step 1
## 1.Read in the data
- Read in the COVID data with data.table::fread() from the NYT GitHub repository
- Read in the state population data with data.table:fread() from the repository
- Merge datasets
...
summary(cv_states)
```
#### The range of dates: 2020-01-21, 2023-03-23. 
#### The range of cases: 1, 12169158. 
#### The range of deaths: 0, 104277.
...
ggplotly(p)
```
#### Generally there is no obvious correlation between population density and new deaths per 100k.
...
cv_states %>% filter(state=="Florida") %>% plot_ly(x = ~date, y = ~new_cases, type = "scatter", mode = "lines") %>% add_lines(x = ~date, y = ~new_deaths, type = "scatter", mode = "lines") %>% layout()

```
#### For the states that had an increase naive_CFR in September, they still increased in the next few months and then decreased over time.
#### The time delay between the peak of cases and the peak of deaths was about one month.
...
cv_states_mat <- cv_states %>% select(state, date, new_cases) %>% dplyr::filter(date>as.Date("2021-06-01"))
...
# Create hover text
cv_per100$hover <- with(cv_per100, paste(state_name, '<br>', "Cases per 100k: ", newper100k, '<br>', "Cases: ", cases, '<br>', "Deaths: ", deaths,'<br>',"naive_CFR:", naive_CFR))
...
fig <- fig %>% colorbar(title = paste0("naive_CFR: ", pick.date), limits = c(0,shadeLimit))
fig <- fig %>% layout(
    title = paste('naive_CFR by State as of ', pick.date, '<br>(Hover for value)'),
    geo = set_map_details
  )
fig_pick.date <- fig
...
fig <- fig %>% colorbar(title = paste0("naive_CFR: ", Sys.Date()), limits = c(0,shadeLimit))
fig <- fig %>% layout(
    title = paste('naive_CFR by State as of', Sys.Date(), '<br>(Hover for value)'),
    geo = set_map_details
  )
fig_Today <- fig
...
```
#### Overall, the naive_CFR in every state has declined more or less over time. (Except Alaska, the navie_CFR in Alaska is the same on these two days.)
